...
if __name__ == "__main__":
    ...
    # load windrose data from files
    speed_filename = 'Amalia_Data/amalia_windrose_speeds.txt'
    frequency_filename = 'Amalia_Data/amalia_windrose_frequencies.txt'
    frequency_data = np.loadtxt(frequency_filename, delimiter='\t')

    windrose_directions = np.arange(0, 360, 5.)
    windrose_directions = 270. - windrose_directions
    for i in range(0, len(windrose_directions)):
        if windrose_directions[i] < 0.:
            windrose_directions[i] += 360.

    windrose_speeds = np.loadtxt(speed_filename, delimiter='\t')

    print frequency_data.shape, windrose_speeds.shape, windrose_directions.shape
    print windrose_directions

    if not optimize_yaw_decoupled:

        nDirections = frequency_data.shape[1]
        print nDirections
        nSpeeds = len(windrose_speeds)

        windrose_speeds_dir_ave = np.zeros(len(windrose_directions))
        windrose_probabilities = np.sum(frequency_data, 0)

        for j in range(0, len(windrose_directions)):
            for i in range(0, len(windrose_speeds)):
                windrose_speeds_dir_ave[j] += windrose_speeds[i]*frequency_data[i, j]
            if windrose_probabilities[j] > 0:
                windrose_speeds_dir_ave[j] /= windrose_probabilities[j]

        print windrose_speeds_dir_ave

        dirPercent = sum(frequency_data, 0)
        print dirPercent
        ...
        # simple small windroses for testing
        ...
        myFloris.windrose_speeds = windrose_speeds_dir_ave
        ...
        myFloris.windrose_directions = windrose_directions
        ...
        print 'turbine X positions (m): %s' % myFloris.turbineX
        print 'turbine Y positions (m): %s' % myFloris.turbineY
        if optimize_yaw:
        ...
        # turbineX = [884.8, 1562.27514372, 2620.51633559, 884.8, 1552.84146636, 2654.4,  884.8, 1498.0259318, 2654.4]
        # turbineY = [884.8, 894.10059291, 887.53016866, 1571.37617195, 1592.61574538, 1596.27536868, 2654.4, 2654.4, 2654.4]
        nDirections = frequency_data.shape[1]
        nSpeeds = windrose_speeds.shape[0]
        print 'nDirections: %s' % nDirections
        print 'nSpeed: %s' % nSpeeds
        ...
        windrose_speeds = np.tile(windrose_speeds[:], (nDirections, 1))
        print 'speeds', windrose_speeds
        ...
        for s in range(0, nSpeeds):

            yaw_data = np.zeros((nDirections, nTurbs))

            yaw_filename = str()

            exec("yaw_filename = 'yaw_files/yaw_allDirections_atSpeed%03.0d_3x3_5D.txt'" % (10.*(windrose_speeds[0, s])))
            # exec("yaw_filename = 'yaw_files/reOptYaw_fromYawAndPosOpt3x3AveSpeedsPerDirection_allDirections_atSpeed%03.0d.txt'" % (10.*(windrose_speeds[0, s])))
            print yaw_filename
            yawfile = open(yaw_filename, 'w')

            for d in range(0, nDirections):

                dirPercent = np.array([frequency_data[s, d]])

                print dirPercent


                # myFloris = floris_assembly_opt(nTurbines=nTurbs, resolution=0)
                myFloris = floris_assembly_opt_AEP(nTurbines=nTurbs, nDirections=1,
                                                   optimize_position=optimize_position, resolution=0.0, optimize_yaw=optimize_yaw,
                                                   use_rotor_components=use_rotor_components, datasize=datasize)
                ...
                # Define flow properties
                myFloris.windrose_frequencies = dirPercent  # cw from north using direction from (standard windrose style)
                myFloris.windrose_speeds = np.array([windrose_speeds[d, s]])
                print 'wind speed (m/s): ', myFloris.windrose_speeds
                myFloris.air_density = 1.1716  # kg/m^3
                myFloris.verbose = False
                ...
                #     print 'yaw (deg) = ', myFloris.yaw
                #     print 'power of each turbine (kW): %s' % myFloris.floris_power_0.wt_power
                print 'yaw (deg) = ', myFloris.yaw_0
                print 'AEP (kWh): %s' % myFloris.AEP

                # save results
                yaw_data[d, :] = myFloris.yaw_0
                print yaw_data

            np.savetxt(yawfile, yaw_data, delimiter='\t')
            print 'saved'
            yawfile.close()
